...
class InventoryRecords extends Component {
    ...
    getProductSummaryCards = () => {
        ...
        for (const name of productNames) {
            ...
            // Calculate per-variant stock based on allocation
            const variants = inventoryProducts.filter(p => p.name === name);
            for (const p of variants) {
                const loc = p.variation_name || '';
                const allocationIn = filteredStockRecords
                    .filter(r => r.product === name && r.type === 'Allocation' && r.location === loc)
                    .reduce((sum, r) => sum + (parseInt(r.quantity) || 0), 0);
                const outgoing = filteredStockRecords
                    .filter(r => r.product === name && r.location === loc && (r.type === 'Outgoing'))
                    .reduce((sum, r) => sum + (parseInt(r.quantity) || 0), 0);
                productMap[name].locations.push({ location: loc, inventoryStock: allocationIn, unitsSold: outgoing });
            }
        }
    ...
    handleIncomingSubmit = async (e) => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                // Store stock does NOT update WooCommerce — allocation does that
