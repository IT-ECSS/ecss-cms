...
class SubmissionSuccessPopup extends Component {
  ...
  handleButtonClick = () => {
    ...
    // Call onClose which handles the entire flow (clear cart, redirect, etc.)
    if (onClose) {
      onClose();
    }
  }
  ...
  render() {
    const { isOpen, selectedLanguage = 'english', isSuccess = true } = this.props;
      ...
      <div className="result-modal-overlay">
        <div className="result-modal-content" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="result-modal-footer">
            <button className="result-modal-button" onClick={this.handleButtonClick}>
              {buttonText}
            </button>
          </div>
        </div>
